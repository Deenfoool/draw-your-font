...
export function createEmptyProject(options = {}) {
  ...
  return {
    ...
    title: options.title || 'Мой рукописный шрифт',
    createdAt: options.createdAt || now,
    updatedAt: now,
    glyphs: [],
    kerning: {},
    font: {
      familyName: options.familyName || options.title || 'Мой рукописный шрифт',
      styleName: 'Regular',
      unitsPerEm: 1000,
      ascent: 800,
      descent: -200,
...
export function defaultGlyphMetrics(character = '') {
  const narrow = /[ГТІI1!.,:;()\[\]{}]/u.test(character);
  const wide = /[ЖШЩМЮW@%&]/u.test(character);
  const punctuation = /[^\p{L}\p{N}]/u.test(character);
  const base = punctuation ? 32 : narrow ? 42 : wide ? 66 : 56;
  return {
    offsetX: 0,
    offsetY: 0,
    scale: 1,
    leftSideBearing: base,
    rightSideBearing: base,
    advanceWidth: null,
  };
}

export function normalizeGlyphRecord(glyph, index = 0) {
  const width = Math.max(1, Math.round(glyph.width || 1));
  const height = Math.max(1, Math.round(glyph.height || 1));
  ...
  return {
    ...
    char: [...String(glyph.char || '').normalize('NFC')][0] || '',
    width,
    height,
    mask,
    guides: {
      capY: Number(glyph.guides?.capY ?? height * 0.12),
      xHeightY: Number(glyph.guides?.xHeightY ?? height * 0.38),
      baselineY: Number(glyph.guides?.baselineY ?? height * 0.82),
      descenderY: Number(glyph.guides?.descenderY ?? height * 0.94),
    },
    metrics: { ...defaultGlyphMetrics(glyph.char), ...(glyph.metrics || {}) },
    quality: glyph.quality || assessGlyphMask(mask, width, height, glyph.guides),
    source: glyph.source || { type: 'unknown' },
  };
}
...
export function projectFromScannedSummary(summary, options = {}) {
  ...
  project.glyphs = summary.glyphs.map(normalizeGlyphRecord);
  project.sourceFiles = [...(options.sourceFiles || [])];
...
export function projectFromSegmentationSource(source, options = {}) {
  ...
  project.glyphs = source.glyphs.map((box, index) => {
    ...
    const x1 = Math.min(source.width - 1, Math.ceil(box.x1) + pad); const y1 = Math.min(source.height - 1, Math.ceil(box.y1) + pad);
    const width = x1 - x0 + 1; const height = y1 - y0 + 1;
    const mask = new Uint8Array(width * height);
    for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) mask[y * width + x] = source.mask[(y0 + y) * source.width + x0 + x] ? 1 : 0;
    return normalizeGlyphRecord({
      id: box.id || `manual-${index + 1}`,
      char: source.labels?.[index] || '',
      width,
      height,
      mask,
      guides: { capY: height * 0.12, xHeightY: height * 0.4, baselineY: height * 0.82, descenderY: height * 0.94 },
      source: { type: 'manual-segmentation', fileName: source.fileName || '' },
    }, index);
  });
  return project;
}
...
export function assessGlyphMask(mask, width, height, guides = null) {
  let count = 0; let minX = width; let maxX = -1; let minY = height; let maxY = -1;
...
export function setMaskPixel(glyph, x, y, value, radius = 1) {
  const cx = Math.round(x); const cy = Math.round(y); const r = Math.max(0, Math.round(radius));
...
export function shiftGlyphMask(glyph, dx, dy) {
  const out = new Uint8Array(glyph.mask.length);
  const ix = Math.round(dx); const iy = Math.round(dy);
  for (let y = 0; y < glyph.height; y += 1) for (let x = 0; x < glyph.width; x += 1) {
    const sx = x - ix; const sy = y - iy;
    if (sx >= 0 && sy >= 0 && sx < glyph.width && sy < glyph.height) out[y * glyph.width + x] = glyph.mask[sy * glyph.width + sx];
  }
  glyph.mask = out;
  glyph.quality = assessGlyphMask(out, glyph.width, glyph.height, glyph.guides);
}

export function scaleGlyphMask(glyph, factor) {
  const scale = clamp(factor, 0.35, 2.5);
  const out = new Uint8Array(glyph.mask.length);
  const cx = (glyph.width - 1) / 2; const baseline = glyph.guides.baselineY;
  for (let y = 0; y < glyph.height; y += 1) for (let x = 0; x < glyph.width; x += 1) {
    const sx = Math.round(cx + (x - cx) / scale);
    const sy = Math.round(baseline + (y - baseline) / scale);
    if (sx >= 0 && sy >= 0 && sx < glyph.width && sy < glyph.height) out[y * glyph.width + x] = glyph.mask[sy * glyph.width + sx];
  }
  glyph.mask = out;
  glyph.quality = assessGlyphMask(out, glyph.width, glyph.height, glyph.guides);
...
function edgeProfiles(glyph) {
  const left = new Float32Array(glyph.height); const right = new Float32Array(glyph.height);
...
export function suggestKerningValue(leftGlyph, rightGlyph, options = {}) {
  const target = Number(options.targetGap ?? 48);
  const leftProfiles = edgeProfiles(leftGlyph); const rightProfiles = edgeProfiles(rightGlyph);
  const rows = Math.max(leftGlyph.height, rightGlyph.height);
  let minimumGap = Infinity; let samples = 0;
  for (let i = 0; i < rows; i += 1) {
    const ly = Math.min(leftGlyph.height - 1, Math.floor(i * leftGlyph.height / rows));
    const ry = Math.min(rightGlyph.height - 1, Math.floor(i * rightGlyph.height / rows));
    const lr = leftProfiles.right[ly]; const rl = rightProfiles.left[ry];
    if (!Number.isFinite(lr) || !Number.isFinite(rl)) continue;
    const leftScale = 700 / Math.max(1, leftGlyph.guides.baselineY - leftGlyph.guides.capY);
    const rightScale = 700 / Math.max(1, rightGlyph.guides.baselineY - rightGlyph.guides.capY);
...
const COMMON_PAIRS = [
  'АВ','АГ','АД','АЛ','АТ','АУ','АЧ','ВА','ГА','ДА','ЛА','ТА','ТО','ТУ','УА','УД','УО','ФА','ЧА','YA',
  'То','Та','Те','Уа','Уо','Ла','Га','Ра','Во','Да','ло','ро','ст','то','тя','уа','фа','ча','ье','ый',
];

export function autoKerning(project) {
  const byChar = new Map(project.glyphs.filter((glyph) => glyph.char).map((glyph) => [glyph.char, glyph]));
  const pairs = {};
  for (const pair of COMMON_PAIRS) {
    const chars = [...pair]; if (chars.length !== 2) continue;
    const left = byChar.get(chars[0]); const right = byChar.get(chars[1]);
    if (!left || !right) continue;
    const value = suggestKerningValue(left, right);
    if (Math.abs(value) >= 8) pairs[`${chars[0]}|${chars[1]}`] = value;
  }
  project.kerning = { ...project.kerning, ...pairs };
  project.updatedAt = nowIso();
  return pairs;
}
...
export function autoMetrics(project) {
  for (const glyph of project.glyphs) {
    const defaults = defaultGlyphMetrics(glyph.char);
    const bbox = glyph.quality?.bbox || assessGlyphMask(glyph.mask, glyph.width, glyph.height, glyph.guides).bbox;
    glyph.metrics.leftSideBearing = defaults.leftSideBearing;
    glyph.metrics.rightSideBearing = defaults.rightSideBearing;
    glyph.metrics.advanceWidth = null;
    if (bbox) {
      const center = (bbox.x0 + bbox.x1) / 2;
      glyph.metrics.offsetX = Math.round((glyph.width / 2 - center) * 0.18);
    }
  }
  project.updatedAt = nowIso();
  return project;
...
function rleEncode(mask) {
  ...
  const bytes = [];
  let value = mask[0] ? 1 : 0; let run = 0;
...
function rleDecode(encoded, expectedLength) {
  ...
  for (let i = 0; i + 2 < binary.length; i += 3) {
    const value = binary.charCodeAt(i) ? 1 : 0;
    const run = (binary.charCodeAt(i + 1) << 8) | binary.charCodeAt(i + 2);
    out.fill(value, offset, Math.min(expectedLength, offset + run)); offset += run;
  }
  if (offset !== expectedLength) throw new Error('Маска проекта повреждена.');
  return out;
}
...
export function serializeProject(project) {
  const plain = {
    ...
    version: PROJECT_VERSION,
    updatedAt: nowIso(),
    glyphs: project.glyphs.map((glyph) => ({ ...glyph, mask: rleEncode(glyph.mask) })),
  };
...
export function deserializeProject(text) {
  ...
  project.kerning = { ...(plain.kerning || {}) };
  project.font = { ...createEmptyProject().font, ...(plain.font || {}) };
  return project;
}

export function validateProject(project) {
  const errors = [];
  if (project?.format !== 'draw-your-font-project') errors.push('Неверный формат проекта.');
  const seen = new Set();
  project?.glyphs?.forEach((glyph, index) => {
    if (!glyph.char) errors.push(`Символ ${index + 1} не подписан.`);
    const codepoint = glyph.char?.codePointAt(0);
    if (codepoint != null && seen.has(codepoint)) errors.push(`Символ «${glyph.char}» повторяется.`);
    if (codepoint != null) seen.add(codepoint);
...
export function projectToFontSource(project) {
  return { kind: 'dyfr-project', project, glyphs: project.glyphs, kerning: project.kerning, font: project.font };
}
