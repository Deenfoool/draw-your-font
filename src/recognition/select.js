...
export function runResidue(mask, width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) { if (mask[y * width + x]) { run += 1; maximum = Math.max(maximum, run); } else run = 0; }
    if (maximum >= width * 0.55) longRows += 1;
  }
  for (let x = 0; x < width; x += 1) {
    let run = 0; let maximum = 0;
    for (let y = 0; y < height; y += 1) { if (mask[y * width + x]) { run += 1; maximum = Math.max(maximum, run); } else run = 0; }
...
function keepTemplateComponents(mask, width, height, expectedChar, guides = {}, options = {}) {
  ...
  if (UPPER_ACCENT_CHARS.has(expectedChar)) {
    for (const component of sorted.slice(1, 5)) {
      if (component.cy < main.y0 && component.area >= Math.max(2, main.area * 0.005) && intervalGap(component.x0, component.x1, main.x0, main.x1) <= width * 0.12 && !kept.includes(component)) kept.push(component);
    }
  }
...
export function scoreGlyphMask(gray, mask, width, height, context = {}) {
  ...
  const components = componentsFromMask(mask, width, height, { minArea: 1, maxComponents: 128 });
  const warnings = [];
  ...
  const baseline = Number.isFinite(guides.baselineY) ? guides.baselineY : height * 0.72;
  let belowBaseline = 0;
  for (let y = Math.max(0, Math.floor(baseline + 1)); y < height; y += 1) for (let x = 0; x < width; x += 1) belowBaseline += mask[y * width + x];
  ...
  const desiredComponents = expectedUpperAccent ? 3 : TWO_PART_PUNCTUATION.has(expectedChar) ? 2 : 1;
  if (components.length > desiredComponents + 3) { score -= Math.min(28, (components.length - desiredComponents - 3) * 4); warnings.push('Слишком много отдельных фрагментов'); }
...
export function candidateDefinitions(options, width, height) {
  const radius = clamp(Math.round(options.backgroundRadius ?? Math.min(width, height) * 0.16), 6, Math.max(6, Math.round(Math.min(width, height) * 0.35)));
...
export function selectBestGlyphMask(grayInput, width, height, context = {}, options = {}) {
  ...
  for (const definition of definitions) {
    ...
    let repaired = bridgeShortGaps(guidesRemoved.mask, width, height, options.repairGap ?? 2);
    ...
    const diagnostic = { method: definition.name, score: quality.score, warnings: quality.warnings, threshold: thresholded.threshold };
    diagnostics.push(diagnostic);
    if (!best || quality.score > best.quality.score || (quality.score === best.quality.score && quality.contrast > best.quality.contrast)) {
      best = { mask: filtered.mask, quality, method: definition.name, localBackground: thresholded.localBackground, removedGuides: guidesRemoved.removed };
    }
  }
