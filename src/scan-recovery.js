...
async function decodeWithWebCodecs(blob) {
  ...
  if (typeof ImageDecoder.isTypeSupported === 'function' && !(await ImageDecoder.isTypeSupported(type))) {
    throw new Error(`WebCodecs не поддерживает ${type}.`);
  }
  ...
  try {
    ...
    if (!image.displayWidth || !image.displayHeight) {
      image.close?.();
      throw new Error('WebCodecs вернул пустой кадр.');
    }
    return image;
  } finally {
    decoder.close?.();
...
export async function decodeImageFile(file) {
  ...
  for (const decoder of decoders) {
    try {
      ...
      return { source, decoder: decoder.name };
    } catch (error) {
      errors.push(`${decoder.name}: ${error?.message || error}`);
    }
  }
...
export function scanTemplateWithRetries(gray, width, height, options = {}) {
  ...
  for (const variant of variants) {
    for (const outputWidth of outputWidths) {
      try {
        const result = scanTemplatePage(variant.gray, width, height, { ...options, outputWidth });
...
export function rectifyFromOuterCorners(gray, width, height, corners, outputWidth = 1260, rotation = 0) {
  ...
  return {
    gray: warpGrayscale(gray, width, height, homography, outputWidth, outputHeight),
    width: outputWidth,
    height: outputHeight,
    homography,
    markers: [],
...
export function scanTemplateFromManualCorners(gray, width, height, corners, options = {}) {
  ...
  for (const rotation of rotations) {
    ...
    if (metadata.valid) {
      ...
      return {
        ...
        plan,
        page,
        glyphs: extractGlyphsFromRectified(rectified, page, options),
        confidence: 0.5,
        recovery: { automatic: false, manualCorners: true, rotation },
      };
    }
  }
  ...
  const metadata = {
    ...
    pageIndex,
    pageCount: activePlan.pageCount,
    totalChars: activePlan.characters.length,
    layoutId: activePlan.layout.id,
    charsetId: activePlan.charsetId,
  };
  return {
    rectified,
    metadata,
    ...
    glyphs: extractGlyphsFromRectified(rectified, page, options),
    confidence: 0.25,
    recovery: { automatic: false, manualCorners: true, rotation, manualPage: true },
  };
}

export function getRecoveryDependencyInfo() {
  return { heicModuleUrl: HEIC_MODULE_URL };
}
