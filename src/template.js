...
export function getLayout(id) {
  return TEMPLATE_LAYOUTS[id] || TEMPLATE_LAYOUTS.balanced;
}
  ...
  for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
    ...
    pageChars.forEach((char, localIndex) => {
      ...
      cells.push({
        ...
        drawingTop,
        drawingHeight,
        capLine: drawingTop + drawingHeight * 0.18,
        xHeightLine: drawingTop + drawingHeight * 0.39,
        baseline,
        descenderLine: drawingTop + drawingHeight * 0.88,
    ...
    pages.push({
      ...
      pageCount,
      title,
      charsetId: options.charsetId || 'custom',
      layout,
...
export function validateTemplatePlan(plan) {
  ...
  for (const page of plan.pages) {
    for (const cell of page.cells) {
      seen.push(cell.char);
      if (cell.x < 0 || cell.y < 0 || cell.x + cell.width > A4_MM.width + 0.001 || cell.y + cell.height > A4_MM.height + 0.001) {
        errors.push(`Ячейка ${cell.index + 1} выходит за границы A4.`);
      }
      if (!(cell.capLine < cell.xHeightLine && cell.xHeightLine < cell.baseline && cell.baseline < cell.descenderLine)) {
        errors.push(`Неверный порядок направляющих в ячейке ${cell.index + 1}.`);
      }
    }
  }
...
function setDashedLine(ctx, dashMm, dpi) {
  ctx.setLineDash(dashMm.map((value) => mmToPx(value, dpi)));
}
...
export function renderTemplatePage(ctx, page, options = {}) {
  ...
  ctx.font = `700 ${Math.round(mmToPx(4.8, dpi))}px Arial, "DejaVu Sans", sans-serif`;
  ctx.fillText(page.title, mmToPx(left, dpi), mmToPx(17, dpi));
  ctx.font = `400 ${Math.round(mmToPx(2.7, dpi))}px Arial, "DejaVu Sans", sans-serif`;
  ...
  ctx.textAlign = 'right';
  ctx.fillStyle = '#111827';
  ctx.font = `700 ${Math.round(mmToPx(3.2, dpi))}px Arial, "DejaVu Sans", sans-serif`;
  ctx.fillText(`Страница ${page.pageNumber} / ${page.pageCount}`, mmToPx(193, dpi), mmToPx(18, dpi));
  ctx.font = `400 ${Math.round(mmToPx(2.5, dpi))}px Arial, "DejaVu Sans", sans-serif`;
  ctx.fillStyle = '#64748b';
  ctx.fillText(`DYF-RU v4 · ${page.layout.columns}×${page.layout.rows}`, mmToPx(193, dpi), mmToPx(23, dpi));
  ctx.textAlign = 'left';

  for (const cell of page.cells) {
    const x = mmToPx(cell.x, dpi);
    const y = mmToPx(cell.y, dpi);
    const w = mmToPx(cell.width, dpi);
    const h = mmToPx(cell.height, dpi);
    ...
    ctx.fillRect(x, y, w, mmToPx(cell.labelBand, dpi));
    ...
    ctx.textAlign = 'left';
    ctx.font = `700 ${Math.round(mmToPx(Math.min(3.8, cell.labelBand * 0.62), dpi))}px Arial, "DejaVu Sans", sans-serif`;
    ctx.fillText(cell.char, mmToPx(cell.x + 2, dpi), mmToPx(cell.y + cell.labelBand * 0.73, dpi));
    ctx.textAlign = 'right';
    ctx.fillStyle = '#64748b';
    ctx.font = `400 ${Math.round(mmToPx(2.1, dpi))}px Arial, "DejaVu Sans", sans-serif`;
    ctx.fillText(String(cell.index + 1), mmToPx(cell.x + cell.width - 1.7, dpi), mmToPx(cell.y + cell.labelBand * 0.7, dpi));

    if (showGuides) {
      ctx.lineWidth = Math.max(1, mmToPx(0.16, dpi));
      ctx.strokeStyle = '#94a3b8';
      setDashedLine(ctx, [1.2, 1.2], dpi);
  ...
  const rulerX = 145;
  const rulerY = footerY - 1.4;
  ctx.strokeStyle = '#111827';
  ctx.lineWidth = Math.max(1, mmToPx(0.25, dpi));
  drawLineMm(ctx, rulerX, rulerY, rulerX + 50, rulerY, dpi);
  for (let i = 0; i <= 5; i += 1) {
    const tickX = rulerX + i * 10;
    drawLineMm(ctx, tickX, rulerY - 1.2, tickX, rulerY + 1.2, dpi);
  }
  ctx.textAlign = 'center';
  ctx.fillText('контрольные 50 мм', mmToPx(rulerX + 25, dpi), mmToPx(footerY + 3.4, dpi));
  ctx.restore();
