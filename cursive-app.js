...
function installCard() {
  ...
  section.innerHTML = `
    <div class="flow-header"><div><p class="eyebrow">Шаг 3.5</p><h2>Связный русский почерк</h2><p>Создаёт isol/init/medi/fina, сохраняет базовую линию и полноценные нижние выносные элементы.</p></div><span class="stage-pill">calt · rlig · curs · desc</span></div>
    <div class="cursive-layout">
      ...
      <div>
        ...
        <div class="cursive-word-preview"><label>Проверка строки<input id="cursivePreviewText" value="дрожь · друг · щука · цифра · уфимец"></label><canvas id="cursiveWordCanvas" width="1100" height="310"></canvas><p id="cursiveSequence"></p></div>
      </div>
    </div>`;
...
function renderWord() {
  ...
  for (const item of items) {
    ...
    const cursive = ensureCursiveProject(project); const generated = generateCursiveFormMask(glyph, item.form, cursive, cursive.glyphs[item.char]); const bodyHeight = Math.max(1, generated.baselineY - generated.xHeightY); const scale = Math.min(1.75, 82 / bodyHeight); context.fillStyle = '#111827';
...
async function buildConnectedFont() {
  ...
  try {
    ...
    setFontStatus('Создаю формы с базовой линией и нижними выносными элементами…');
    ...
    byId('fontOutputStats').innerHTML = `<strong>${built.glyphs.length}</strong> глифов · GSUB/GPOS · вертикаль ${built.layout.metrics.ascent}/${built.layout.metrics.descent} · TTF ${(built.ttf.length/1024).toFixed(1)} КБ · WOFF2 ${(woff2.length/1024).toFixed(1)} КБ`;
    if (progress) progress.value = 6; setFontStatus('Связный шрифт с нижними элементами собран и проверен.', 'ok'); return state.outputs;
